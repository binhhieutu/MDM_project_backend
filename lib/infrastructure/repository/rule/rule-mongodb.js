...
const ruleInitData = require("../../database/mongodb/init/rule-data.json");

module.exports = class RuleMongodbRepository {
  constructor () {
    this.model = mongoose.model(collections.rule, schemas.rule);
  }

  async initData () {
    const count = await this.model.countDocuments();
    if (count === 0) {
      this.model.create(ruleInitData);
    }
  }
