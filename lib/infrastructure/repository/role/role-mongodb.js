...
const roleInitData = require("../../database/mongodb/init/role-data.json");

module.exports = class RoleMongodbRepository {
  constructor () {
    this.model = mongoose.model(collections.role, schemas.role);
  }

  async initData () {
    const count = await this.model.countDocuments();
    if (count === 0) {
      this.model.create(roleInitData);
    }
  }
