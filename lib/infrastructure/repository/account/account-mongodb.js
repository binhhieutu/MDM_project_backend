...
const accountInitData = require("../../database/mongodb/init/account-data.json");

module.exports = class AccountMongodbRepository {
  constructor () {
    this.model = mongoose.model(collections.account, schemas.account);
  }

  async initData () {
    const count = await this.model.countDocuments();
    if (count === 0) {
      this.model.create(accountInitData);
    }
  }
  ...
  async selectMany ({ condition, ability, sort, project, limit, skip }) {
